...
def get_transcript(video_url):
  ...
  stream = yt.streams.filter(only_audio=False)[0]
  stream.download(filename="temp.mp4")
  timestamps = get_keyframes(json.dumps(transcript))
  frames = extract_frames_from_video(timestamps)
  return title, thumbnail, text, frames
